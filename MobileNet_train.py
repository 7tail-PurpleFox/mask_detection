...
def build_dataset(data_dir, img_size=(224, 224), val_split=0.2, batch_size=32, seed=9527):
    ...
    # apply MobileNetV3Large preprocessing (same semantics as previous code)
...
def build_model(num_classes, img_size=(224, 224)):
    ...
    # Ensure float32 outputs when using mixed precision to maintain numerical stability
    headModel = layers.Dense(num_classes, activation="softmax", dtype="float32")(headModel)
...
def parse_args():
    parser = argparse.ArgumentParser(description="Train MobileNetV3Large-based mask detector")
    ...
    parser.add_argument("--mixed_precision", action="store_true", help="enable mixed precision (fp16) on supported GPUs")
...
def main():
    ...
    # Optionally enable mixed precision to reduce GPU memory usage
